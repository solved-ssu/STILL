...
import { createPage, getPageById } from "./pages";
...
describe("comment repository", () => {
  ...
  beforeEach(() => {
    ...
    publishedPageId = createPage(
      database,
      { title: "댓글을 받을 문서", icon: "C", excerpt: "", topicSlug: "algorithm", subtopicSlug: null, content: [], status: "published" },
  ...
  it("공개 문서에 댓글을 작성하고 시간순으로 조회한다", () => {
    const first = createComment(database, publishedPageId, "  이 부분의 시간복잡도가 궁금합니다.  ", {
      studentId: "20261234",
      role: "member",
    }, new Date("2026-08-14T01:00:00.000Z"));
    const second = createComment(database, publishedPageId, "답변을 기다리고 있습니다.", {
      studentId: "20265678",
      role: "member",
    ...
    expect(listCommentsByPage(database, publishedPageId)).toEqual([
    ...
    expect(getPageById(database, publishedPageId, "20261234")?.commentCount).toBe(2);
  });
  ...
  it("댓글은 작성자 또는 관리자만 삭제한다", () => {
    const comment = createComment(database, publishedPageId, "삭제 권한 확인", { studentId: "20261234", role: "member" });
  ...
  it("최근 1분 동안 작성한 댓글이 5개면 제한한다", () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      createComment(database, publishedPageId, `댓글 ${index}`, { studentId: "20261234", role: "member" }, now);
    }
